...
export class QCollection {
  ...
  get selected() {
    return this._selected || '_default';
  }
  ...
  remove(scheme, select_previous = true) {
    ...
    if (found_index !== -1) {
      const id = scheme.id;
      if (this.selected === id && select_previous) {
        this.selected = this._schemes[Math.max(0, found_index - 1)]?.id || "";
        this.apply_selected();
      }
   ...
   */
  get_selected() {
    return this.find_by_id(this.selected) || false;
  }
  
  apply_selected() {
    let scheme = this.get_selected();
    if (scheme) scheme.apply();
  }
  ...
  equals(qcollection) {
    ...
    if (qcollection.selected !== this.selected) return false;
